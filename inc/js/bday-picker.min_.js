 ...
 */
(function(d){
	var a={"short":["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"],
	...
	d.fn.birthdaypicker=function(g) {
			...
			return this.each(function(){					
				if(g) {d.extend(h,g)}
				var r=d("<fieldset class='birthday-picker'></fieldset>"),
					q=d("<select class='birth-year' name='birth[year]'></select>"),
					p=d("<select class='birth-month' name='birth[month]'></select>"),
					m=d("<select class='birth-day' name='birth[day]'></select>");
				
				if(h.legend){d("<legend>"+h.legend+"</legend>").appendTo(r)}
				...
				if(h.placeholder){
					d("<option value='0'>Año:</option>").appendTo(q);
					d("<option value='0'>Mes:</option>").appendTo(p);
					d("<option value='0'>Día:</option>").appendTo(m);
					n=null
				}
				...
				if(h.hiddenDate){
					d("<input type='hidden' name='birthdate' id='birthdate' />").val(n).appendTo(r)
				}
				...
				r.change(function(){
					...
					if((u*x*v)!=0){
						...
						if (h.nacimiento) {
							n2=u+"-"+x+"-"+v+" 00:00:00"
							document.getElementById(h.nacimiento).value = n2;
						}
