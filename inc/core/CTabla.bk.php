...
class Tabla {
	...
	*/				
	...
	function Edicion($id) {
		...
		$this->resultados = 0;
		$this->SQL = '';
		$this->SQL = "SELECT * FROM ".$this->nombre." WHERE ".$this->primario."=".$id;
		$this->SQLCOUNT = "SELECT COUNT(*) FROM ".$this->nombre." WHERE ".$this->primario."=".$id;
		$this->Open();
	...
	function FiltrarCampoStr($nombre,$nested='',$escondido='') {
		...
		if ($escondido=='' || $escondido=='label') {			
			if ($campo['nreferencias']==0) {
				...
				if (($referencia['tipo']=='directa') or ($referencia['tipo']=='autoreferencia')) {
					for( $i = 0; $i<$campo['nreferencias']; $i++) {
						$referencia_mas = $referencias[$i];
								if ($referencia_mas['tipo'] == 'directa') {
...
//				$resstr.= '<OPTION VALUE="" '.$selected.' class="filtro">Vacio - - - -</OPTION>';
				...
				if ($this->nresultados>0) {
					...
					while ($row = $this->Fetch( "", $RAW_RESULTS=true, $campos_sql ) ) {
						...
						if ( $this->nresultados<200 ) {
							$resstr.= '<OPTION VALUE="'.$row[$elalias.".".$referencia['clave']].'"  '.$selected.' class="combo">'.$rowmuestras.'</OPTION>';
						}
					}
					
					if ( !( $this->nresultados < 200 ) ) {
						$resstr.= '<input ID="'.$formpoint.'_e_'.$campo['nombre'].'_str" NAME="_e_'.$campo['nombre'].'_str" type="text" value="'.$value_str.'">';
						$resstr.= '<input ID="'.$formpoint.'_e_'.$campo['nombre'].'" NAME="_e_'.$campo['nombre'].'" type="text" value="'.$value_id.'">';
					}
