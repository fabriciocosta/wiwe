...
<form name="consultar" method="post" action="consulta.php">
			<!--FILTROS-->
				<table border="0" cellpadding="0" cellspacing="10">
					...
					<tr>
						<td><? $tabla->FiltrarCampo('ID_SECCION','/*SPECIAL*/padres.ID IN ('.$_usuariosecciones_.')');?></td>
						<td rowspan="5" valign="bottom"><a href="javascript:consultar();"><img src="../images/buscar.gif" alt="" width="50" height="25" border="0" onMouseDown="javascript:showimg('../images/buscar_down.gif');" onMouseOut="javascript:showimg('../images/buscar.gif');"></a></td>
					</tr>
					<tr>
						<td><? $tabla->FiltrarCampo('ID_TIPOSECCION'); ?></td>
					</tr>
					<tr>
						<td><? $tabla->FiltrarCampo('NOMBRE'); ?></td>
					</tr>
					<tr>
						<td><? $tabla->FiltrarCampo('CARPETA'); ?></td>
					</tr>
					<tr>					
						<td><? $tabla->FiltrarCampo('DESCRIPCION'); ?></td>						
					</tr>						
				</table>
			<!--FIN FILTROS-->
...
<? 
	if ($_consulta_=='si') {
		...
		$tabla->FiltrarSQL('ID_TIPOSECCION');
		$tabla->FiltrarSQL('NOMBRE');
		$tabla->FiltrarSQL('CARPETA');			
