...
<form name="consultar" method="post" action="consulta.php">
			<!--FILTROS-->
				<table border="0" cellpadding="0" cellspacing="10">
					...
					<tr>
						<td><? $tabla->FiltrarCampo('ID_CONTENIDO'); ?></td>						
					...
					<tr>
						<td><? $tabla->FiltrarCampo('ID_TIPODETALLE'); ?></td>						
					</tr>
					<tr>
						<td ><? $tabla->FiltrarCampo('DETALLE'); ?></td>
					</tr>
					<tr>
						<td ><? $tabla->FiltrarCampo('ML_DETALLE'); ?></td>
					</tr>
					<tr>
						<td ><? $tabla->FiltrarCampo('TXTDATA'); ?></td>
					</tr>										
					<tr>
						<td ><? $tabla->FiltrarCampo('ML_TXTDATA'); ?></td>
					</tr>					
				</table>
			<!--FIN FILTROS-->
...
<? 
	if ($_consulta_=='si') {
		$tabla->LimpiarSQL();
		$tabla->FiltrarSQL('ID_CONTENIDO');
		$tabla->FiltrarSQL('ID_TIPODETALLE');
		$tabla->FiltrarSQL('DETALLE');
		$tabla->FiltrarSQL('ML_DETALLE');
		$tabla->FiltrarSQL('TXTDATA');
		$tabla->FiltrarSQL('ML_TXTDATA');
